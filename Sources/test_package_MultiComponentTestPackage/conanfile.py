...
class TestPackage(ConanFile):
    ...
    packages = ["MultiComponentTestPackage"]
    #cpp_package_components = ["AComponent", "BComponent"]

    def setPackageOptions(self, packages):
    ...
    def configure(self):
        self.setPackageOptions(self.packages)
    ...
    def build(self):
        ...
        for package in self.packages:
            package_dir = self.deps_cpp_info[package].rootpath + "/lib/cmake/{0}".format(package)
            cmake_generate_command += " -D{0}_DIR=\"{1}\"".format(package, package_dir)
